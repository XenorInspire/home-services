...
$associate = $hm_database->getAssociateById($_POST['aid']);
if ($associate == NULL) {
...
if ($_GET['mode'] == 1) {
    ...
    $old_password = hash('sha512', 'ChrysaleadProject' . $_POST['old_password']);

    if ($associate->getPassword() != $old_password) {
    ...
    system('python3 mail/mail.py ' . 4 . ' ' . $associate->getEmail());
